...
import { getDeltaLabel, safeFormat } from '../../../lib/utils'
...
function CompareColumn({
  ...
  const displayDate = safeFormat(session.date, 'MMM d, yyyy', getDateLocale(i18n.language))
        ...
        >
          {sessions.map((item) => (
            <option key={item.id} value={item.id}>
              {t(`tasks.${item.taskLabel}`)} · {safeFormat(item.date, 'MMM d, yyyy', getDateLocale(i18n.language))} · {t('common.minutesFormat', { value: item.durationMin })}
            </option>
          ))}
        </select>
